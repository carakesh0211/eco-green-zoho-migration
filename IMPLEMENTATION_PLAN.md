...
- **2026-09-15 (increment 2 started)** -- Team-operable console workstreams
  ...
  merge.
- **2026-09-15 (increment 2 deployed)** — Team-operable console on Development, build `9f9d374`, verified by the lead from the deployed URL (37/37 live checks) and `node --test` **610/610**:
  - Branch Control Dashboard over 351 seeded summaries (PILOT01 real + 350 synthetic; `EXPECTED_BRANCH_COUNT=351`): page 1 of 8 in ~130 ms end-to-end (~60 ms store query for the ≤351-row summary table fetched in two ZCQL pages), filters/sort/search/CSV export server-side, page size capped at 200, one page per request.
  - Team & assignments: directory (config bootstrap + `app_users`), invite → INVITED → ACTIVE, `VERSION_CONFLICT` on stale writes, `SOD_VIOLATION` (same person as operator and approver; preparer cannot approve own batch), reassignment requires a reason, history from audit events, workload counts; server-side branch scope proven on every `/api` route (scope-matrix test + live probes: PILOT01-scoped operator sees `total=1`, 403 elsewhere; bots 403 on team/admin/export).
  - Authentication: `AUTH_MODE=token,catalyst`; the AppSail origin serves `/__catalyst/sdk/init.js` and a hosted login page (verified live), sessions map to ACTIVE `app_users` by email; bearer path unchanged for bots. Development tokens rotated; the previously reported operator token returns 401.
  - Zoho Books connection page: NOT CONNECTED, six independent controls, `Connect` refused (`BOOKS_NOT_CONFIGURED`), `Test` refused (`NOT_CONNECTED`), zero live Books calls; `live.js` endpoints verified against official docs (`/banktransfers` → `/banktransactions` bug fixed).
  - Platform defects found by live verification and fixed with regression tests: Catalyst ZCQL `OFFSET` is 1-based (a 0-based paginator repeated one row per 300-row page — the 351-row read returned 352); every paged SELECT now carries an `ORDER BY …, ROWID` tiebreak; the dashboard-summary refresh hook is best-effort and handles summary-only branches; assignment history ordering is deterministic.
  - Docs: `docs/CAPACITY_REVIEW.md` (approval required before real ingestion), `docs/CATALYST_AUTH.md`, `docs/ZOHO_BOOKS_API_REFERENCES.md`; DEPLOYMENT.md §4d lists the remaining manual/owner actions (enable Catalyst Authentication + add users, register the Books OAuth client, read-only authorization, capacity approval).
