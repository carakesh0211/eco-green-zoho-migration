...
- **2026-09-15 (Stratus live)** — Stratus activated and proven on Development:
  ...
  - Redeployed (`buildSha 8aab133`, health `archiveAdapter: stratus`, `archiveStatus: ENABLED`) and re-ran the live smoke: **7/7 PASS** (put, exists, get sha-verified, same-bytes idempotent, different-bytes rejected with `IMMUTABLE_CONFLICT`, original intact after conflict, unwritten uri absent) — run `smoke-95c3618a31e0` under `SMOKE01/`. The two synthetic `SMOKE01/` objects (one per smoke run) remain in the bucket as evidence; delete only on owner instruction.
